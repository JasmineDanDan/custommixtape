...
const isMixMaker = location.pathname.includes("mixtape-maker");
let activeAudio;
let activeSongId;
let toastTimer;
...
function MixApp() {
  ...
  if (!hasCardHash && giftScenes.includes(requestedScene)) { state.mood=requestedScene; state.step=2; }
  ...
  function actionBar() {
    return `<div class="editor-action-wrap">${state.step===3?'<p class="finish-note">When you finish, you\'ll get a shareable link. Your recipient does not need an account.</p>':''}<div class="action-bar editor-actions"><button class="secondary" data-action="back" ${state.step===0?'disabled':''}>Back</button><button class="primary" data-action="${state.step===3?'finish':'next'}">${state.step===3?'Finish':'Next'}</button></div></div>`;
  }
  const examples=[
    {id:'birthday',title:'Birthday for a Friend',occasion:'Birthday',recipient:'Emma',sender:'Alex',note:'For every tiny reason you make life brighter. Happy birthday!',theme:'yellow',stickers:['birthday-cake','ribbon-gift-box'],songs:['happy-birthday','happy-phone','contigo-es-diferente','three-kisses']},
    {id:'anniversary',title:'Love & Anniversary',occasion:'Anniversary',recipient:'Jamie',sender:'Morgan',note:'Four little songs for our favorite memories together.',theme:'brick',stickers:['pink-rose','satin-bow'],songs:['three-kisses','contigo-es-diferente','mundanely-romantic','happy-phone']},
    {id:'comfort',title:'A Little Comfort',occasion:'A little comfort',recipient:'Sam',sender:'Riley',note:'A quiet reminder that I am thinking of you today.',theme:'sky',stickers:['little-sun','latte'],songs:['mundanely-romantic','happy-phone','contigo-es-diferente','three-kisses']}
  ];
  function exampleUrl(example) { return `${location.pathname}#card=${encode({...starter(),...example,step:3,generated:true,mood:'All'})}`; }
  function exampleCards() {
    return examples.map(example=>`<article class="mix-example-card"><div class="mix-example-art" style="${getThemeStyle(example.theme)}"><div class="mix-example-note"><small>To: ${esc(example.recipient)}</small><small>From: ${esc(example.sender)}</small><strong>${esc(example.occasion)}</strong></div><div class="mix-example-tape"><span>Made for you</span><i></i><i></i><b>4 tracks</b></div></div><div class="mix-example-copy"><span class="example-badge">${esc(example.occasion)}</span><h3>${esc(example.title)}</h3><p>Made for: ${esc(example.recipient)}<br />4 songs · A personal note</p><a class="text-link" href="${esc(exampleUrl(example))}" data-example-url="${esc(exampleUrl(example))}">Open this mixtape</a></div></article>`).join('');
  }
  function makerPage() {
    const toolPrefix=isMixMaker?'../':'./';
    return `${commonHeader('mix')}
      <main class="maker-page">
        <section class="intro maker-intro"><div><div class="eyebrow">Free browser tool</div><h1>Free Mixtape Maker</h1><p>Create a personalized digital mixtape with a retro cassette design, stickers, songs, and a personal note. Finish it as a shareable music gift.</p><button class="primary maker-hero-action" data-action="focus-maker">Start Making</button></div></section>
        <section class="tool-steps" aria-labelledby="maker-steps-heading"><div class="tool-section-heading"><div class="eyebrow">How to use the tool</div><h2 id="maker-steps-heading">Make your mixtape in four steps.</h2></div><ol class="tool-step-grid"><li><strong>Choose a cassette look</strong><span>Pick from four visual backgrounds.</span></li><li><strong>Add up to three stickers</strong><span>Choose small details for the person or moment.</span></li><li><strong>Pick and arrange up to four songs</strong><span>Preview songs, filter by scene, and set the order.</span></li><li><strong>Write a note and share your gift</strong><span>Add To, From and a personal message.</span></li></ol></section>
        <section class="workspace maker-workspace" id="mixtape-maker"><aside class="preview-column"><div class="preview-shell">${preview()}</div></aside><section class="editor"><p class="maker-guide">Make your mixtape in four small steps.</p><div class="step-tabs">${['Look','Stickers','Songs','Note'].map((name,i)=>`<button class="step-tab ${state.step===i?'active':''}" data-step="${i}" ${i===3&&!state.songs.length?'disabled aria-disabled="true" title="Choose at least one song first"':''}><small>${i+1}</small> ${name}</button>`).join('')}</div><div class="editor-panel">${panel()}</div>${actionBar()}</section></section>
        <section class="tool-features" aria-labelledby="maker-features-heading"><div class="tool-section-heading"><div class="eyebrow">What you can make</div><h2 id="maker-features-heading">A small tool with everything this gift needs.</h2></div><div class="tool-feature-grid"><article><h3>Cassette looks</h3><p>Choose Meadow Sprig, Lemon Windowpane, Coral Ribbon or Powder Constellation.</p></article><article><h3>Stickers</h3><p>Add up to three stickers, including a birthday cake, gift box, tulip bouquet, coffee, rainbow and rose.</p></article><article><h3>Songs</h3><p>Choose from the site's openly licensed songs, preview them, select up to four and adjust their order. Scene filters include Birthday, For a friend, Love &amp; anniversary and A little comfort.</p></article><article><h3>Personal note</h3><p>Write a To name up to 40 characters, a From name up to 40 characters, and a note up to 140 characters.</p></article><article><h3>Share link</h3><p>When you finish, the maker creates one share link. The recipient can open it in a browser without registering and press play to hear the songs.</p></article></div></section>
        <section class="tool-scenes" aria-labelledby="maker-scenes-heading"><div class="tool-section-heading"><div class="eyebrow">Start with a moment</div><h2 id="maker-scenes-heading">Choose a moment first, then select songs that fit it.</h2></div><div class="tool-scene-grid"><article><h3>Birthday</h3><p>Build a small celebration with a warm closing note.</p></article><article><h3>For a friend</h3><p>Arrange songs around a shared memory or inside joke.</p></article><article><h3>Love &amp; anniversary</h3><p>Tell a short story through four meaningful tracks.</p></article><article><h3>A little comfort</h3><p>Send a gentle sequence for a harder day.</p></article></div></section>
        <section class="tool-faq" aria-labelledby="maker-faq-heading"><div class="tool-section-heading"><div class="eyebrow">Tool FAQ</div><h2 id="maker-faq-heading">Questions before you start.</h2></div><div class="faq"><details open><summary>How many songs can I add?</summary><p>You can choose up to four songs and arrange them in the order you want them played.</p></details><details><summary>Can I preview the songs?</summary><p>Yes. Use the play button beside a song to hear its available preview before adding it.</p></details><details><summary>Can I change the song order?</summary><p>Yes. Once songs are selected, use the up and down controls in Your cassette order.</p></details><details><summary>How many stickers can I choose?</summary><p>You can add up to three stickers to the cassette card.</p></details><details><summary>How long can my note be?</summary><p>The To and From fields allow up to 40 characters each. The personal note allows up to 140 characters.</p></details><details><summary>Does the recipient need an account?</summary><p>No. Anyone with the share link can open the finished mixtape in a browser.</p></details><details><summary>Can I share the finished mixtape?</summary><p>Yes. Finish the card, then share the generated link with your recipient.</p></details><details><summary>Where is my unfinished mixtape saved?</summary><p>Your unfinished work is saved in this browser while you make it. It is not uploaded as a public gift until you finish and share it.</p></details></div></section>
        <section class="tool-links" aria-labelledby="maker-links-heading"><div class="tool-section-heading"><div class="eyebrow">Keep exploring</div><h2 id="maker-links-heading">Make the gift your own.</h2></div><nav aria-label="Related mixtape pages"><a href="${toolPrefix}make-a-mixtape">Learn how to make a mixtape</a><a href="${toolPrefix}custom-mixtape">Create a custom mixtape</a><a href="${toolPrefix}mixtape-ideas">Browse mixtape ideas</a><a href="${toolPrefix}custom-j-cards">Make a matching J card</a></nav></section>
      </main>${commonFooter()}`;
  }
  function giftView(isOwner) {
    const url=writeHash({...state,generated:true});
    const ownerControls=isOwner?`<div class="share-panel"><label>Share link</label><div><input readonly value="${esc(url)}" data-share-url="${esc(url)}" aria-label="Share link" /><button class="primary" data-action="share">Copy</button></div><p class="privacy-note share-privacy">Anyone with your share link can see this gift's content. Please do not include private or sensitive information.</p></div><div class="action-bar"><button class="secondary" data-action="edit">Edit gift</button><button class="secondary" data-action="reset">Start over</button></div>`:'';
  ...
  function render() {
    ...
    if (state.generated) {
      let isOwner=false;
      try { isOwner=sessionStorage.getItem('made-mixtape-hash')===location.hash; } catch {}
      giftView(isOwner);
      return;
    }
    if (!root.querySelector(isMixMaker ? '.maker-intro' : '.mix-intro')) {
      if (isMixMaker) {
        root.innerHTML=makerPage();
        bind();
        return;
      }
      root.innerHTML = `${commonHeader('mix')}
      <section class="intro mix-intro"><div><h1>Create a Mixtape for Someone You Love</h1><p>Choose a cassette look, add up to four songs, write a little note, and share a playable music gift with one link.</p><button class="primary mix-hero-action" data-action="focus-maker">Make Your Mixtape</button></div></section>
      <section class="workspace" id="mixtape-maker"><aside class="preview-column"><div class="preview-shell">${preview()}</div></aside>
      <section class="editor"><p class="maker-guide">Make it in four small steps: Choose a look · Add up to 3 stickers · Pick up to 4 songs · Write a note</p><div class="step-tabs">${['Look','Stickers','Songs','Note'].map((name,i)=>`<button class="step-tab ${state.step===i?'active':''}" data-step="${i}" ${i===3&&!state.songs.length?'disabled aria-disabled="true" title="Choose at least one song first"':''}><small>${i+1}</small> ${name}</button>`).join('')}</div><div class="editor-panel">${panel()}</div>${actionBar()}</section></section>
      <section class="mix-examples" aria-labelledby="mix-examples-heading"><div class="mix-section-heading"><div class="eyebrow">Made to open</div><h2 id="mix-examples-heading">See What They Will Receive</h2><p>Each example is a real, playable gift card with songs and a note inside.</p></div><div class="mix-example-grid">${exampleCards()}</div></section>
      <section class="mix-value" aria-labelledby="mix-value-heading"><div class="mix-section-heading"><div class="eyebrow">Why it feels personal</div><h2 id="mix-value-heading">A Small Music Gift, Made Personal</h2></div><div class="mix-value-points"><article><h3>Pick songs with a reason</h3><p>Choose up to four openly licensed songs and place them in the order you want someone to hear them.</p></article><article><h3>Make the card feel like them</h3><p>Choose a cassette backdrop and up to three little stickers for a birthday, friendship, anniversary, or quiet everyday moment.</p></article><article><h3>Give them a moment to open</h3><p>Add a note, create a link, and send a gift they can open in their browser without creating an account.</p></article></div></section>
      <section class="mix-scenes" aria-labelledby="mix-scenes-heading"><div class="mix-section-heading"><div class="eyebrow">Choose a starting point</div><h2 id="mix-scenes-heading">Start With the Moment</h2><p>Pick an occasion, then choose songs that fit.</p></div><div class="scene-grid"><article class="scene-card"><h3>A birthday surprise</h3><p>Make a small celebration they can replay.</p><button class="secondary" data-scene="Birthday">Make this mixtape</button></article><article class="scene-card"><h3>For your best friend</h3><p>Put a shared feeling into four songs.</p><button class="secondary" data-scene="For a friend">Make this mixtape</button></article><article class="scene-card"><h3>Love &amp; anniversary</h3><p>Hold onto a favorite chapter together.</p><button class="secondary" data-scene="Love & anniversary">Make this mixtape</button></article><article class="scene-card"><h3>A little comfort</h3><p>Send a gentle note for a harder day.</p><button class="secondary" data-scene="A little comfort">Make this mixtape</button></article><article class="scene-card"><h3>Just because</h3><p>No occasion needed for a thoughtful hello.</p><button class="secondary" data-scene="All">Make this mixtape</button></article></div></section>
      <section class="mix-howto" aria-labelledby="mix-howto-heading"><div class="mix-howto-heading"><div class="eyebrow">How it works</div><h2 id="mix-howto-heading">Three Small Steps, One Personal Gift</h2></div><div class="mix-howto-steps"><article><span>01</span><h3>Customize your cassette</h3><p>Choose a cassette design and a few stickers that fit the person, the music, or the occasion.</p></article><article><span>02</span><h3>Add songs and a personal note</h3><p>Preview the available songs, arrange them in your chosen order, and write a short message.</p></article><article><span>03</span><h3>Share your digital gift</h3><p>Finish the card and send one memorable gift link.</p></article></div></section>
      <section class="mix-faq" aria-labelledby="mix-faq-heading"><div class="mix-faq-heading"><div class="eyebrow">FAQ</div><h2 id="mix-faq-heading">A few useful things to know.</h2></div><div class="faq"><details open><summary>Does the recipient need an account?</summary><p>No. Anyone with the gift link can open the mixtape in their browser.</p></details><details><summary>Can the recipient play the songs?</summary><p>Yes. The shared gift keeps the selected openly licensed songs and their order. Playback starts after the recipient presses play.</p></details><details><summary>Where does my mixtape live before I share it?</summary><p>Your mixtape stays in this browser while you make it. The gift link carries the card settings when you are ready to share.</p></details><details><summary>Can I make a matching cassette J card?</summary><p>Yes. Use <a href="custom-j-cards">Custom J cards</a> to create a printable cassette insert with an album title, dedication, and track list.</p></details><details><summary>How many songs can I add?</summary><p>You can choose up to four songs and arrange them in the order you want them played.</p></details><details><summary>Is the mixtape maker free?</summary><p>Yes. You can create and share a mixtape for free.</p></details></div></section>
      <section class="mix-explore" aria-labelledby="mix-explore-heading"><div class="mix-section-heading"><div class="eyebrow">Explore more</div><h2 id="mix-explore-heading">Make It Your Own</h2></div><nav aria-label="Make It Your Own"><a href="mixtape-maker">Free Mixtape Maker</a><a href="make-a-mixtape">How to Make a Mixtape</a><a href="custom-mixtape">Create a Custom Mixtape</a><a href="mixtape-ideas">Mixtape Ideas</a><a href="custom-j-cards">Make a Cassette J-Card</a></nav></section>${commonFooter()}`;
    } else {
      ...
      root.querySelector('.editor-action-wrap').outerHTML=actionBar();
    }
    bind();
  }
  function panel() {
  ...
  function bind() {
    ...
    root.querySelectorAll('[data-scene]').forEach(btn=>btn.onclick=()=>{state.mood=btn.dataset.scene;state.step=2;render();requestAnimationFrame(()=>root.querySelector('#mixtape-maker')?.scrollIntoView({behavior:'smooth',block:'start'}));});
    root.querySelectorAll('[data-example-url]').forEach(link=>link.onclick=event=>{event.preventDefault();location.href=link.dataset.exampleUrl;location.reload();});
  ...
  function bindGift() {
    ...
    root.querySelector('[data-action="edit"]')?.addEventListener('click',()=>{state.generated=false;writeHash(state);render();});
     root.querySelector('[data-action="reset"]')?.addEventListener('click',()=>{ if(confirm('Start over with a blank mixtape gift?')) { location.hash=''; state=starter(); clearDraft('tape-note-mixtape-draft'); try { sessionStorage.removeItem('made-mixtape-hash'); sessionStorage.removeItem('made-mixtape'); } catch {} render(); } });
    root.querySelector('[data-action="play-all"]')?.addEventListener('click',()=>{const song=selectedSongs()[0];if(song)playPreview(song);});
    root.querySelectorAll('[data-gift-play]').forEach(button=>button.addEventListener('click',()=>{const song=selectedSongs().find(item=>item.id===button.dataset.giftPlay);if(song)playPreview(song);}));
...
function JCardApp() {
  ...
  function bind(){
    ...
    bindPanel();
    const downloadFaq=root.querySelector('.jcard-faq .faq details:nth-of-type(2) p');
...
function injectJCardExtras() {
  if (!isJCard || document.querySelector('.jcard-map')) return;
  const faq=document.querySelector('.jcard-faq');
  if (!faq) return;
  faq.insertAdjacentHTML('beforebegin', `<section class="jcard-map" aria-labelledby="jcard-map-heading"><div><div class="eyebrow">Know the parts</div><h2 id="jcard-map-heading">What a J Card Holds</h2><p>A J card gives a mixtape a paper home. Each area has a job when you are building the insert.</p></div><div class="jcard-map-diagram" aria-label="J card parts"><div><strong>Front cover</strong><span>Album title and visual mood</span></div><div><strong>Spine</strong><span>Recipient, sender and identity</span></div><div><strong>Back panel</strong><span>Dedication and keepsake details</span></div><div><strong>Side A / Side B</strong><span>Track list in listening order</span></div></div></section><section class="jcard-cases" aria-labelledby="jcard-cases-heading"><div class="jcard-section-heading"><div class="eyebrow">Made for the moment</div><h2 id="jcard-cases-heading">J Card Ideas for a Personal Gift</h2><p>Use the same template for different kinds of mixtape.</p></div><div class="jcard-case-grid"><article><h3>Birthday J-card</h3><p>Pair a bright paper style with a short birthday dedication.</p></article><article><h3>Love letter J-card</h3><p>Use Side A and Side B to tell a small story through the track list.</p></article><article><h3>Best friend J-card</h3><p>Add a shared phrase, familiar songs and a playful cover.</p></article><article><h3>Anniversary J-card</h3><p>Arrange the tracks as a small timeline of memories and future plans.</p></article><article><h3>Thank-you J-card</h3><p>Pair a specific dedication with songs that recognize what this person has done.</p></article><article><h3>Comfort gift J-card</h3><p>Keep the layout quiet and let the note carry the warmth.</p></article></div><p class="jcard-digital-link">Want a playable digital gift too? <a href="../">Create a custom mixtape with songs, a note and a shareable link.</a></p></section>`);
}
